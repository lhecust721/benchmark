...
- 使用文件路径图像模式时，缓存图片存储于 `{tool_root_path}/ais_bench/datasets/RefCOCOg/data/temp_save_images/`。需确保推理服务能够访问该路径下的本地文件。以 vLLM 为例，启动服务时需添加参数 `--allowed-local-media-path {tool_root_path}/ais_bench/datasets/RefCOCOg/data`。
